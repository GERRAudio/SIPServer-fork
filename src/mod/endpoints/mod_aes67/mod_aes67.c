													...
													switch_originate_flag_t flags, switch_call_cause_t *cancel_cause)
{
	...
	if ((tech_pvt = (private_t *)switch_core_session_alloc(*new_session, sizeof(private_t))) != 0) {
		...
		switch_mutex_init(&tech_pvt->flag_mutex, SWITCH_MUTEX_NESTED,
						  switch_core_session_get_pool(*new_session)); /// new stream, new mutex
...
static void *init_ptp(int domain, char *iface)
{
	...
	if (!gst_clock_wait_for_sync(GST_CLOCK(clock), timeout)) {
		...
		gst_object_unref(GST_OBJECT(clock));	//added to deref extras - check
		return NULL;
	}
...
SWITCH_MODULE_SHUTDOWN_FUNCTION(mod_aes67_shutdown)
{
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE,
					  ...
					  g_alloc_counts.memory, g_alloc_counts.messages, g_alloc_counts.objs, g_alloc_counts.chars);
