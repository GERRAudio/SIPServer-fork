...
#define STREAM_READER_TRYLOCK(s) (!g_atomic_int_get(&(s)->reloading) && g_rw_lock_reader_trylock(&(s)->rwlock))
...
static audio_stream_t *find_audio_stream(udp_sock_t *indev, udp_sock_t *outdev, int already_locked);
static audio_stream_t *get_audio_stream(udp_sock_t *indev, udp_sock_t *outdev);
...
static switch_status_t destroy_audio_stream(udp_sock_t *indev, udp_sock_t *outdev);
...
static audio_stream_t *find_audio_stream(udp_sock_t *indev, udp_sock_t *outdev, int already_locked)
{
	...
	while (cur_stream != NULL) {
		if (outdev == NULL || is_sock_equal(cur_stream->outdev, outdev)) { // null check on outdev added
...
static void destroy_shared_audio_streams()
{
	...
	for (hi = switch_core_hash_first(globals.sh_streams); hi; hi = switch_core_hash_next(&hi)) {
		...
		switch_core_hash_delete(globals.sh_streams, key); // added to clear table - check
	}
...
static switch_status_t destroy_audio_stream(udp_sock_t *indev, udp_sock_t *outdev)
{
	...
	return destroy_actual_stream(stream); // check
	// return SWITCH_STATUS_SUCCESS;
}
...
static switch_status_t channel_kill_channel(switch_core_session_t *session, int sig)
{
	...
	switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_DEBUG, "%s CHANNEL KILL, sig %d\n",
...
static switch_status_t channel_endpoint_read(private_t *tech_pvt, switch_frame_t **frame, char *session_id)
{
	int bytes = 0;
	int samples = 0;
	audio_endpoint_t *endpoint = tech_pvt->audio_endpoint;
...
static switch_status_t channel_endpoint_write(private_t *tech_pvt, switch_frame_t *frame)
{
	...
	if (!switch_test_flag(tech_pvt, TFLAG_IO)) { return SWITCH_STATUS_SUCCESS; }

	if (STREAM_READER_TRYLOCK(endpoint->out_stream)) {
		if (!endpoint->out_stream->stream) { return SWITCH_STATUS_FALSE; }
...
static int create_shared_audio_stream(shared_audio_stream_t *stream); //fwd decl
static int take_stream_channel(shared_audio_stream_t *stream, int index, int input, const char *session_id)
{
	int rc = 0;
	if (!stream) { return rc; }
	...
	if (input) {
		if (stream->inchan_used[index] && !stream->multiple_listen) {
			rc = -1;
			goto done;
		}
													...
													switch_originate_flag_t flags, switch_call_cause_t *cancel_cause)
{
	...
	if (!(*new_session =
			  switch_core_session_request_uuid(aes67_endpoint_interface, SWITCH_CALL_DIRECTION_OUTBOUND, flags, pool,
	...
	if (outbound_profile->destination_number &&
		...
		if (switch_core_codec_init(&tech_pvt->read_codec, "L16", NULL, NULL, sample_rate, codec_ms, 1,
		...
		// hardcode to Raw 16bit
		if (switch_core_codec_init(&tech_pvt->write_codec, "L16", NULL, NULL, sample_rate, codec_ms, 1,
								   SWITCH_CODEC_FLAG_ENCODE | SWITCH_CODEC_FLAG_DECODE, NULL,
								   NULL) != SWITCH_STATUS_SUCCESS) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Can't load codec?\n");
			goto error;
		...
		switch_snprintf(name, sizeof(name), "aes67/endpoint-%s", endpoint_name);
...
error:
	if (tech_pvt) {
		if (tech_pvt->read_timer.interval) { switch_core_timer_destroy(&tech_pvt->read_timer); }
		if (tech_pvt->write_timer.interval) { switch_core_timer_destroy(&tech_pvt->write_timer); }
...
static void gst_logger(GstDebugCategory *category, GstDebugLevel level, const gchar *file, const gchar *function,
...
#define MAX_PATH_LEN 10000
	...
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
					  "Resolving the absolute path to Media (GStreamer) path: %s\n", media_dir);
...
#endif
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
						  "Resolved the abolute path to Media (GStreamer) path as %s\n", media_dir);
		struct stat st_buf;
		int ret = -1;
		if (((ret = stat(media_dir, &st_buf)) == 0) && (st_buf.st_mode & S_IFDIR)) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
							  "Setting it as environment variable: GST_PLUGIN_PATH\n");
			switch_setenv("GST_PLUGIN_PATH", media_dir, TRUE);
		} else {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
							  "Can't access %s, error: %s. Not overriding GST_PLUGIN_PATH\n", media_dir,
							  strerror(errno));
		}
	} else {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
						  "Failed to resolve the Media (GStreamer) path"
#ifdef _WIN32
						  " \n");
#else
						   "%s\n",
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	aes67_endpoint_interface = switch_loadable_module_create_interface(*module_interface, SWITCH_ENDPOINT_INTERFACE);
...
static void emit_ptp_gm_change(gboolean synced)
{
	switch_event_t *event;
...
static gboolean ptp_stats_cb(guint8 d, const GstStructure *stats, gpointer user_data)
{
...
static switch_status_t load_streams(switch_xml_t streams, switch_bool_t reload)
{
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	switch_xml_t param, mystream;
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "%soading streams ...\n", reload ? "Rel" : "L");
	...
	for (mystream = switch_xml_child(streams, "stream"); mystream; mystream = mystream->next) {
		...
		for (param = switch_xml_child(mystream, "param"); param; param = param->next) {
			...
			if (!strcmp(var, "tx-address")) {
				...
				if (stream->outdev == NULL) switch_malloc(stream->outdev, sizeof(udp_sock_t));
				...
				if (stream->indev == NULL) switch_malloc(stream->indev, sizeof(udp_sock_t));
...
static shared_audio_stream_t *check_stream(char *streamstr, int check_input, int *chanindex)
{
	...
	*chan = 0;
	...
	if (check_input && stream->indev == NULL) { return NULL; }

	if (!check_input && stream->outdev == NULL) { return NULL; }
...
static switch_bool_t endpoint_in_use(audio_endpoint_t *endp)
{
	...
	for (hi = switch_core_hash_first(globals.call_hash); hi; hi = switch_core_hash_next(&hi)) {
		...
		if (tech_pvt->audio_endpoint == endp) {
			...
			if (state == CCS_ACTIVE) {
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_NOTICE, " %s's session is ACTIVE",
								  tech_pvt->audio_endpoint->name);
				return TRUE;
			}
		}
	}
...
static switch_bool_t endpoint_compare(audio_endpoint_t *curr, audio_endpoint_t *new)
{
...
static switch_status_t load_endpoints(switch_xml_t endpoints, switch_bool_t reload)
{
	switch_status_t status = SWITCH_STATUS_SUCCESS;
	switch_xml_t param, myendpoint;
	switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG, "%soading endpoints ...\n", reload ? "Rel" : "L");
	...
	for (myendpoint = switch_xml_child(endpoints, "endpoint"); myendpoint; myendpoint = myendpoint->next) {
		...
		if (endpoint) {
			if (!reload) {
				...
				switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "The endpoint '%s' is in use, can't change\n",
								  endpoint_name);
				continue;
			}
...
static switch_status_t load_globals(switch_xml_t cfg)
{
...
static switch_status_t load_config(void)
{
	...
	if ((streams = switch_xml_child(cfg, "streams"))) { load_streams(streams, FALSE); }

	if ((endpoints = switch_xml_child(cfg, "endpoints"))) { load_endpoints(endpoints, FALSE); }
...
int open_audio_stream(g_stream_t **stream, udp_sock_t *indev, udp_sock_t *outdev)
{
	...
	if (!indev && !outdev) {
		*stream = NULL;
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "indev or outdev not defined\n");
		return -1;
	}
	data.direction = 0;
...
static audio_stream_t *create_audio_stream(udp_sock_t *indev, udp_sock_t *outdev)
{
	...
	if (ret != 0) {
		switch_core_timer_destroy(&(stream->write_timer));			//added check
		switch_safe_free(stream);
...
audio_stream_t *get_audio_stream(udp_sock_t *indev, udp_sock_t *outdev)
{
	audio_stream_t *stream = NULL;
	if (outdev == NULL) {
...
static switch_status_t list_shared_streams(switch_stream_handle_t *stream)
{
	...
	for (hi = switch_core_hash_first(globals.sh_streams); hi; hi = switch_core_hash_next(&hi)) {
		...
		stream->write_function(stream,
							   ...
							   s->name, s->indev ? s->indev->ip_addr : "None", s->indev ? s->indev->port : 0,
...
static switch_status_t reload_config()
{
	...
	if (SWITCH_STATUS_SUCCESS != (status = switch_xml_reload(&reload_err))) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "reload conf failed: %s\n", reload_err);
		return SWITCH_STATUS_GENERR;
	}
	...
	if ((streams = switch_xml_child(cfg, "streams"))) { load_streams(streams, TRUE); }

	if ((endpoints = switch_xml_child(cfg, "endpoints"))) { load_endpoints(endpoints, TRUE); }
