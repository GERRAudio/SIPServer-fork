...
#define G_ALLOC_WRAP_INC(counter, name, t, p)   inline void AL_##name(t p) { if (p) g_alloc_counts.counter++; }

// --- Macro for decrement-only wrappers (for manual tracking) ---
#define G_ALLOC_WRAP_DEC(counter, name, t, p)   inline void DA_##name(t p) { if (p) g_alloc_counts.counter--; }
