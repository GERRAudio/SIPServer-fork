...
// if theuy are not wrapped, the comiler will flag them 
...
#define G_ALLOC_WRAP_ALLOC(ret_type, func, counter, ...)         \
    static inline ret_type AL_##func(__VA_ARGS__) {              \
        ret_type _ret = func(__VA_ARGS__);                       \
...
G_ALLOC_WRAP_ALLOC(GstPad*, gst_element_get_static_pad, objs, GstElement *element, const gchar *name)
G_ALLOC_WRAP_ALLOC(GstPad*, gst_pad_get_peer, objs, GstPad *pad)
G_ALLOC_WRAP_ALLOC(GstElement*, gst_bin_get_by_name, objs, GstBin *bin, const gchar *name)
G_ALLOC_WRAP_ALLOC(GstElement*, gst_element_factory_make, objs, const gchar *factoryname, const gchar *name)
...
G_ALLOC_WRAP_ALLOC(GstSample*, gst_app_sink_try_pull_sample, samples, GstAppSink *appsink, guint64 timeout)
G_ALLOC_WRAP_ALLOC(GstSample*, gst_app_sink_pull_preroll, samples, GstAppSink *appsink)
G_ALLOC_WRAP_ALLOC(GstSample*, gst_sample_new, samples, GstBuffer *buffer, GstCaps *caps, GstSegment *segment, GstStructure *info)
G_ALLOC_WRAP_REF(gst_sample_ref, samples, GstSample *)

// --- caps wrappers ---
G_ALLOC_WRAP_FREE(gst_caps_unref, GstCaps *)
...
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_copy_nth, caps, const GstCaps *caps, guint nth)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_copy_and_set_caps_features, caps, const GstCaps *caps, guint index, const GstCapsFeatures *features)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_from_string, caps, const gchar *string)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_merge, caps, GstCaps *caps1, GstCaps *caps2)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_merge_structure, caps, GstCaps *caps, GstStructure *structure)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_normalize, caps, GstCaps *caps)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_merge_structure_full, caps, GstCaps *caps, GstStructure *structure, GstCapsFeatures *features)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_type_find_helper_for_buffer, caps, GstObject *obj, GstBuffer *buf, GstCaps *filter)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_type_find_helper_for_buffer_with_caps, caps, GstObject *obj, GstBuffer *buf, GstCaps *caps, GstCaps *filter)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_intersect, caps, const GstCaps *caps1, const GstCaps *caps2)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_intersect_full, caps, const GstCaps *caps1, const GstCaps *caps2, GstCapsIntersectMode mode)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_union, caps, const GstCaps *caps1, const GstCaps *caps2)
G_ALLOC_WRAP_ALLOC(GstCaps*, gst_caps_subtract, caps, const GstCaps *minuend, const GstCaps *subtrahend)
...
G_ALLOC_WRAP_ALLOC(GstCapsFeatures*, gst_caps_features_new, features, const gchar *feature1, ...)
...
G_ALLOC_WRAP_ALLOC(GstMemory*, gst_allocator_alloc, memories, GstAllocator *allocator, gsize size, GstAllocationParams *params)
G_ALLOC_WRAP_ALLOC(GstMemory*, gst_memory_new_wrapped, memories, GstMemoryFlags flags, gpointer data, gsize maxsize, gsize offset, gsize size, GDestroyNotify notify, gpointer user_data)
