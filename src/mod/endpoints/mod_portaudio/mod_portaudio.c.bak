...
typedef enum { GFLAG_NONE = 0, GFLAG_EAR = (1 << 0), GFLAG_MOUTH = (1 << 1), GFLAG_RING = (1 << 2) } GFLAGS;
...
PaError open_audio_stream(PABLIO_Stream **stream, const PaStreamParameters *inputParameters,
...
static void pa_com_uninit(switch_core_session_t *session, PaWinUtilComInitializationResult *res)
{
	...
	if (res->state != PAWINUTIL_COM_INITIALIZED) { return; }
	...
	if (res->initializingThreadId != currentThreadId) {
		switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
						  "ASIO uninit thread does not match!\n");
		return;
	}
...
static switch_status_t channel_on_init(switch_core_session_t *session)
{
	...
	if (session) {
		if ((channel = switch_core_session_get_channel(session))) { switch_channel_set_flag(channel, CF_AUDIO); }
	}
...
static switch_status_t channel_on_routing(switch_core_session_t *session)
{
	...
	if (switch_test_flag(tech_pvt, TFLAG_OUTBOUND)) {
		...
		if (!tech_pvt->audio_endpoint && switch_test_flag(tech_pvt, TFLAG_OUTBOUND) &&
			!switch_test_flag(tech_pvt, TFLAG_AUTO_ANSWER)) {

			add_pvt(tech_pvt, PA_SLAVE);

			ring_file = globals.ring_file;
			if ((val = switch_channel_get_variable(channel, "pa_ring_file"))) { ring_file = val; }

			if (switch_test_flag((&globals), GFLAG_RING)) { ring_file = NULL; }
			switch_set_flag_locked((&globals), GFLAG_RING);
			if (ring_file) {
				if (switch_core_file_open(&fh, ring_file, globals.read_codec.implementation->number_of_channels,
					...
					switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(session), SWITCH_LOG_ERROR,
									  "Cannot open %s, disabling ring file!\n", ring_file);
					ring_file = NULL;
				}
			}
		}
		...
		while (switch_channel_get_state(channel) == CS_ROUTING && !switch_channel_test_flag(channel, CF_ANSWERED) &&
			...
			if (switch_micro_time_now() - last >= waitsec) {
				...
				if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_RINGING) ==
					...
					switch_event_add_header_string(event, SWITCH_STACK_BOTTOM, "call_id",
												   tech_pvt->call_id); /* left behind for backwards compatability */
			...
			if (ring_file) {
				...
				if (globals.ring_stream &&
					(!switch_test_flag(globals.call_list, TFLAG_MASTER) ||
					 (!globals.no_ring_during_call &&
					  globals.main_stream !=
						  globals.ring_stream))) { // if there is a ring stream and not an active call or if there is an
												   // active call and we are allowed to ring during it AND the ring
												   // stream is not the main stream
					WriteAudioStream(globals.ring_stream->stream, abuf, (long)olen, 0,
									 &globals.ring_stream->write_timer);
				}
			} else {
				switch_yield(10000);
			}
	...
	if (switch_test_flag(tech_pvt, TFLAG_OUTBOUND)) {
		if (!switch_test_flag(tech_pvt, TFLAG_ANSWER) && !switch_channel_test_flag(channel, CF_ANSWERED)) {
...
static switch_status_t validate_ring_audio_stream()
{
	if (globals.ringdev == -1) { return SWITCH_STATUS_SUCCESS; }
	if (globals.ring_stream) {
	...
	if (globals.ring_stream) { return SWITCH_STATUS_SUCCESS; }
...
static switch_status_t destroy_actual_stream(audio_stream_t *stream)
{
	...
	if (globals.ring_stream == stream) { globals.ring_stream = NULL; }
...
static switch_status_t channel_endpoint_read(audio_endpoint_t *endpoint, switch_frame_t **frame)
{
	...
	samples = ReadAudioStream(endpoint->in_stream->stream, endpoint->read_frame.data,
							  STREAM_SAMPLES_PER_PACKET(endpoint->in_stream), endpoint->inchan, &endpoint->read_timer);
...
static void update_level(private_t *tech_pvt, switch_frame_t *frame, uint8_t rx)
{
	...
	samples = tech_pvt->audio_endpoint ? tech_pvt->audio_endpoint->read_codec.implementation->actual_samples_per_second
									   : globals.read_codec.implementation->actual_samples_per_second;
	if (scount >= samples) {
		if (rx) {
			switch_event_t *event;
			if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_CALL_AUDIO_LEVEL) ==
				SWITCH_STATUS_SUCCESS) {
				double rxdb;
				double txdb;
				float avg_rx;
				...
				switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(tech_pvt->session), SWITCH_LOG_DEBUG,
								  "firing level event at scount=%u with rate %u rx=%f tx=%f\n", scount, samples, avg_rx,
								  avg_tx);
										  ...
										  switch_io_flag_t flags, int stream_id)
{
	...
	*frame = NULL;
	...
	if (!switch_test_flag(tech_pvt, TFLAG_IO)) { goto cng_wait; }
	...
	samples = ReadAudioStream(globals.main_stream->stream, globals.read_frame.data,
							  globals.read_codec.implementation->samples_per_packet, 0, &globals.read_timer);
...
normal_return:
	if (*frame) { update_level(tech_pvt, *frame, 1); }
	return status;
...
static switch_status_t channel_endpoint_write(audio_endpoint_t *endpoint, switch_frame_t *frame)
{
	...
	if (!endpoint->master) { return SWITCH_STATUS_SUCCESS; }
	if (switch_test_flag(endpoint->master, TFLAG_HUP)) { return SWITCH_STATUS_FALSE; }
	if (!switch_test_flag(endpoint->master, TFLAG_IO)) { return SWITCH_STATUS_SUCCESS; }
	WriteAudioStream(endpoint->out_stream->stream, (short *)frame->data, (int)(frame->datalen / sizeof(SAMPLE)),
										   ...
										   switch_io_flag_t flags, int stream_id)
{
	...
	if (tech_pvt->audio_endpoint) { return channel_endpoint_write(tech_pvt->audio_endpoint, frame); }
	...
	if (globals.main_stream) {
		if (switch_test_flag((&globals), GFLAG_EAR)) {
			WriteAudioStream(globals.main_stream->stream, (short *)frame->data, (int)(frame->datalen / sizeof(SAMPLE)),
							 0, &(globals.main_stream->write_timer));
		}
		status = SWITCH_STATUS_SUCCESS;
	}
...
static switch_status_t channel_answer_channel(switch_core_session_t *session) { return SWITCH_STATUS_SUCCESS; }
...
static int release_stream_channel(shared_audio_stream_t *stream, int index, int input)
{
	...
	for (i = 0; i < stream->channels; i++) {
		if (stream->inchan_used[i] || stream->outchan_used[i]) { destroy_stream = 0; }
	}
	if (destroy_stream) { destroy_shared_audio_stream(stream); }
													...
													switch_originate_flag_t flags, switch_call_cause_t *cancel_cause)
{
	...
	if (!(*new_session =
			  switch_core_session_request_uuid(portaudio_endpoint_interface, SWITCH_CALL_DIRECTION_OUTBOUND, flags,
											   pool, switch_event_get_header(var_event, "origination_uuid")))) {
		return retcause;
	}
	...
	if (outbound_profile->destination_number &&
		!strncasecmp(outbound_profile->destination_number, "endpoint", sizeof("endpoint") - 1)) {
		endpoint = NULL;
		...
		if (!endpoint_name) {
			switch_log_printf(SWITCH_CHANNEL_SESSION_LOG(*new_session), SWITCH_LOG_CRIT,
							  "No portaudio endpoint specified\n");
			goto error;
		}
		endpoint_name++;
		...
		samples_per_packet = endpoint->in_stream ? STREAM_SAMPLES_PER_PACKET(endpoint->in_stream)
		...
		if (switch_core_timer_init(&endpoint->read_timer, globals.timer_name, codec_ms, samples_per_packet,
		...
		if (switch_core_timer_init(&endpoint->write_timer, globals.timer_name, codec_ms, samples_per_packet,
								   module_pool) != SWITCH_STATUS_SUCCESS) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "failed to setup read timer for endpoint '%s'!\n",
							  endpoint->name);
			goto error;
		...
		if (switch_core_codec_init(&endpoint->read_codec, "L16", NULL, NULL, sample_rate, codec_ms, 1,
		...
		if (switch_core_codec_init(&endpoint->write_codec, "L16", NULL, NULL, sample_rate, codec_ms, 1,
								   SWITCH_CODEC_FLAG_ENCODE | SWITCH_CODEC_FLAG_DECODE, NULL,
								   NULL) != SWITCH_STATUS_SUCCESS) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Can't load codec?\n");
			goto error;
		...
		if (var_event && (endpoint_answer = (switch_event_get_header(var_event, "endpoint_answer")))) {
			if (switch_true(endpoint_answer)) { switch_set_flag(tech_pvt, TFLAG_AUTO_ANSWER); }
		} else {
...
error:
	if (endpoint) {
		if (!endpoint->master) {
			if (endpoint->read_timer.interval) { switch_core_timer_destroy(&endpoint->read_timer); }
			if (endpoint->write_timer.interval) { switch_core_timer_destroy(&endpoint->write_timer); }
			if (endpoint->read_codec.codec_interface) { switch_core_codec_destroy(&endpoint->read_codec); }
			if (endpoint->write_codec.codec_interface) { switch_core_codec_destroy(&endpoint->write_codec); }
		}
		switch_mutex_unlock(endpoint->mutex);
	}
...
SWITCH_MODULE_LOAD_FUNCTION(mod_portaudio_load)
{
	...
	*module_interface = switch_loadable_module_create_module_interface(pool, modname);
	...
	switch_console_set_complete("add pa devices");
	...
	for (mystream = switch_xml_child(streams, "stream"); mystream; mystream = mystream->next) {
		...
		for (param = switch_xml_child(mystream, "param"); param; param = param->next) {
			...
			if (!strcmp(var, "indev")) {
				...
				if (devval < 0) {
					switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "Invalid indev specified for stream '%s'\n",
									  stream_name);
					stream->indev = -1;
					continue;
				}
		...
		if (stream->indev < 0 && stream->outdev < 0) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "You need at least one device for stream '%s'\n",
							  stream_name);
			continue;
		}
...
static int check_stream_compat(shared_audio_stream_t *in_stream, shared_audio_stream_t *out_stream)
{
	...
	if (in_stream->sample_rate != out_stream->sample_rate) { return -1; }
	if (in_stream->codec_ms != out_stream->codec_ms) { return -1; }
	return 0;
}
...
static shared_audio_stream_t *check_stream(char *streamstr, int check_input, int *chanindex)
{
	...
	*chan = 0;
	...
	if (check_input && stream->indev < 0) { return NULL; }

	if (!check_input && stream->outdev < 0) { return NULL; }
...
static switch_status_t load_endpoints(switch_xml_t endpoints)
{
	...
	for (myendpoint = switch_xml_child(endpoints, "endpoint"); myendpoint; myendpoint = myendpoint->next) {
		...
		if (!endpoint->in_stream && !endpoint->out_stream) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR, "You need at least one stream for endpoint '%s'\n",
							  endpoint_name);
			continue;
		}
...
static switch_status_t load_config(void)
{
	...
	if (globals.indev < 0) {
		...
		if (globals.indev > -1) {
			...
			if (globals.unload_device_fail) { status = SWITCH_STATUS_GENERR; }
		}
	}
	...
	if (globals.ringdev < 0) {
		if (globals.outdev > -1) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_DEBUG,
							  "Invalid or no ring device configured, using output device as ring device\n");
...
static int get_dev_by_number(char *numstr, int in)
{
	...
	if (end_ptr == numstr || number < 0) { return -1; }
...
/*******************************************************************/
static switch_status_t play_dev(switch_stream_handle_t *stream, int outdev, char *file, const char *max_seconds,
								const char *no_close)
{
	switch_file_handle_t fh = {0};
	int samples = 0;
	...
	if (!strcasecmp(file, "ringtest")) { file = globals.ring_file; }
	if (outdev == -1) {
	...
	if (switch_core_file_open(&fh, file, globals.read_codec.implementation->number_of_channels,
							  globals.read_codec.implementation->actual_samples_per_second,
							  SWITCH_FILE_FLAG_READ | SWITCH_FILE_DATA_SHORT, NULL) != SWITCH_STATUS_SUCCESS) {
	...
	if (globals.call_list) {
		...
		if (switch_test_flag(globals.call_list, TFLAG_MASTER) &&
			globals.main_stream->outdev == outdev) { /*so we are the active stream so we need to dupe it basically */
	...
	while (switch_core_file_read(&fh, abuf, &olen) == SWITCH_STATUS_SUCCESS) {
		if (globals.destroying_streams || !audio_stream->stream) { break; }
	...
	switch_core_file_close(&fh);
	if (!globals.call_list && (!no_close || strcasecmp(no_close, "no_close"))) { destroy_audio_streams(); }

	seconds = wrote / globals.read_codec.implementation->actual_samples_per_second;
	stream->write_function(stream, "playback test [%s] %d second(s) %d samples @%dkhz", file, seconds, wrote,
						   globals.read_codec.implementation->actual_samples_per_second);
...
static switch_status_t devlist(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	if (numDevices < 0) { return SWITCH_STATUS_SUCCESS; }
	...
	if (argv[0] && !strcasecmp(argv[0], "xml")) {
		...
		for (i = 0; i < numDevices; i++) {
			...
			if (globals.indev == i) {
				if (prev) { stream->write_function(stream, ","); }
...
static int dump_info(int verbose)
{
	...
	for (i = 0; i < numDevices; i++) {
		...
		if (defaultDisplayed) { switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "** | "); }
		/* print device info fields */
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Name: %s\n", deviceInfo->name);
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Host: %s | ",
		...
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO, "Default input latency: %.3f | ",
		...
		if (inputParameters.channelCount > 0) {
			...
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
							  "half-duplex 16 bit %d channel input rates:", inputParameters.channelCount);
			PrintSupportedStandardSampleRates(&inputParameters, NULL);
		}

		if (outputParameters.channelCount > 0) {
			switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_INFO,
							  "half-duplex 16 bit %d channel output rates:", outputParameters.channelCount);
						  ...
						  const PaStreamParameters *outputParameters)
{
	if (inputParameters->device != -1) {
		return OpenAudioStream(stream, inputParameters, outputParameters, globals.sample_rate, paClipOff,
							   globals.read_codec.implementation->samples_per_packet, globals.dual_streams);
	}
	return OpenAudioStream(stream, NULL, outputParameters, globals.sample_rate, paClipOff,
						   globals.read_codec.implementation->samples_per_packet, 0);
}

PaError open_shared_audio_stream(shared_audio_stream_t *shstream, const PaStreamParameters *inputParameters,
								 const PaStreamParameters *outputParameters)
{
	PaError err;
	if (inputParameters->device != -1) {
		...
		err = OpenAudioStream(&shstream->stream, NULL, outputParameters, shstream->sample_rate, paClipOff,
							  STREAM_SAMPLES_PER_PACKET(shstream), 0);
	}
	if (err != paNoError) { shstream->stream = NULL; }
	return err;
}
...
static int create_shared_audio_stream(shared_audio_stream_t *shstream)
{
	...
	if (err != paNoError) {
		switch_log_printf(SWITCH_CHANNEL_LOG, SWITCH_LOG_ERROR,
						  "Can't open audio device (indev = %d, outdev = %d, error = %s)\n", inputParameters.device,
						  outputParameters.device, Pa_GetErrorText(err));
...
static switch_status_t list_endpoints(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	if (argv[0] && !strcasecmp(argv[0], "xml")) {
		...
		stream->write_function(stream, "\t</endpoints>\n</xml>\n");
	} 
	else {
		...
		for (hi = switch_core_hash_first(globals.endpoints); hi; hi = switch_core_hash_next(&hi)) {
			...
			stream->write_function(stream, "%s> instream: %s, outstream: %s\n", e->name,
								   e->in_stream ? e->in_stream->name : "(none)",
...
static switch_status_t prepare_stream(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	if (*argv[1] == '#') { devval2 = get_dev_by_number(argv[1] + 1, 0); }
	if (devval2 == -1) {
...
static switch_status_t switch_stream(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	if (*argv[0] == '#') { devval = get_dev_by_number(argv[0] + 1, 1); }
	if (devval == -1) {
...
static switch_status_t looptest(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	globals.stream_in_use = 1;
	for (i = 0; i < 400; i++) {
		if (globals.destroying_streams || !globals.main_stream->stream) { break; }
		if ((samples =
				 ReadAudioStream(globals.main_stream->stream, globals.read_frame.data,
								 globals.read_codec.implementation->samples_per_packet, 0, &globals.read_timer))) {
			WriteAudioStream(globals.main_stream->stream, globals.read_frame.data, (long)samples, 0,
...
static switch_status_t set_ringfile(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	if (create_codecs(0) == SWITCH_STATUS_SUCCESS) {
		switch_file_handle_t fh = {0};
		if (switch_core_file_open(&fh, argv[0], globals.read_codec.implementation->number_of_channels,
								  globals.read_codec.implementation->actual_samples_per_second,
								  SWITCH_FILE_FLAG_READ | SWITCH_FILE_DATA_SHORT, NULL) == SWITCH_STATUS_SUCCESS) {
			switch_core_file_close(&fh);
...
static switch_status_t switch_call(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	if (tech_pvt) {
		if (tech_pvt == globals.call_list && !tech_pvt->next) { one_call = 1; }

		if (!one_call) { remove_pvt(tech_pvt); }
		add_pvt(tech_pvt, PA_MASTER);
	...
	if (!zstr(callid)) {
		if ((tp = switch_core_hash_find(globals.call_hash, callid))) {
			if (switch_test_flag(tp, TFLAG_ANSWER)) {
				...
				if (tp != globals.call_list) { remove_pvt(tp); }
...
static switch_status_t do_flags(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	for (x = 1; x < argc; x++) {
		flag_str = argv[x];
		for (p = flag_str; *p; p++) { *p = (char)tolower(*p); }

		if (strstr(flag_str, "ear")) { flags |= GFLAG_EAR; }
		if (strstr(flag_str, "mouth")) { flags |= GFLAG_MOUTH; }
	}

	if (!strcasecmp(action, "on")) {
		if (flags & GFLAG_EAR) { switch_set_flag((&globals), GFLAG_EAR); }
		if (flags & GFLAG_MOUTH) { switch_set_flag((&globals), GFLAG_MOUTH); }
	} else if (!strcasecmp(action, "off")) {
		if (flags & GFLAG_EAR) { switch_clear_flag((&globals), GFLAG_EAR); }
		if (flags & GFLAG_MOUTH) { switch_clear_flag((&globals), GFLAG_MOUTH); }
	} else {
		goto bad;
	}
...
desc:
	...
	if (!x) { stream->write_function(stream, "none"); }
...
static switch_status_t list_calls(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	for (tp = globals.call_list; tp; tp = tp->next) {
		...
		stream->write_function(stream, "%s %s [%s (%s)] %s\n", tp->call_id, switch_channel_get_name(channel), cid_name,
							   cid_num, switch_test_flag(tp, TFLAG_MASTER) ? "active" : "hold");
	}
	switch_mutex_unlock(globals.pvt_lock);
...
static switch_status_t place_call(char **argv, int argc, switch_stream_handle_t *stream)
{
	...
	if ((session = switch_core_session_request(portaudio_endpoint_interface, SWITCH_CALL_DIRECTION_INBOUND, SOF_NONE,
											   NULL)) != 0) {
		private_t *tech_pvt;
		switch_channel_t *channel;
		...
		if (!zstr(argv[1])) { dialplan = argv[1]; }

		if (!zstr(argv[2])) { cid_num = argv[2]; }

		if (!zstr(argv[3])) { cid_name = argv[3]; }

		if (!zstr(argv[4])) { tech_pvt->sample_rate = atoi(argv[4]); }

		if (!zstr(argv[5])) { tech_pvt->codec_ms = atoi(argv[5]); }

		switch_find_local_ip(ip, sizeof(ip), NULL, AF_INET);

		if ((tech_pvt->caller_profile =
				 switch_caller_profile_new(switch_core_session_get_pool(session), NULL, dialplan, cid_name, cid_num, ip,
										   NULL, NULL, NULL, modname, context, dest)) != 0) {
			char name[128];
			switch_snprintf(name, sizeof(name), "portaudio/%s",
							tech_pvt->caller_profile->destination_number ? tech_pvt->caller_profile->destination_number
																		 : modname);
			switch_channel_set_name(channel, name);
		...
		if (validate_main_audio_stream() == SWITCH_STATUS_SUCCESS) {
			...
			if (switch_core_session_thread_launch(tech_pvt->session) != SWITCH_STATUS_SUCCESS) {
				...
				if (switch_event_create_subclass(&event, SWITCH_EVENT_CUSTOM, MY_EVENT_MAKE_CALL) ==
					SWITCH_STATUS_SUCCESS) {
					char buf[512];
					switch_channel_event_set_data(channel, event);
...
SWITCH_STANDARD_API(pa_cmd)
{
	...
	if (stream->param_event) { http = switch_event_get_header(stream->param_event, "http-host"); }
	...
	if (http) {
		...
		if (action) {
			if (strlen(action) == 1) {
				...
				cmd = cmd_buf;
			} else if (!strcmp(action, "hangup") || !strcmp(action, "list") || !strcmp(action, "devlist") ||
					   !strcmp(action, "answer")) {
				cmd = action;
			}
		}

		if (zstr(cmd)) { goto done; }
	...
	} else if (!strcasecmp(argv[0], "devices")) {
		func = list_shared_streams;
	...
	if (func) {
		if (http) { stream->write_function(stream, "<pre>"); }

		switch_mutex_lock(globals.pa_mutex);
		func(&argv[lead], argc - lead, stream);
		status =
			SWITCH_STATUS_SUCCESS; /*if func was defined we want to always return success as the command was found */
		switch_mutex_unlock(globals.pa_mutex);

		if (http) { stream->write_function(stream, "\n\n</pre>"); }
	}

done:
	if (http) {
		stream->write_function(stream, "<br><br><table align=center><tr><td><center><form method=post>\n"
