...
switch_status_t validate_keygen_signed_license(const char *key_content, const char *file_path, license_key_t *key_info)
{
	...
	*dot = '\0';
	...
	/* Strip "key/" prefix if present (Keygen format) */
	if (strncmp(payload_b64, "key/", 4) == 0) {
		payload_b64 += 4;
		licensing_log(SWITCH_LOG_DEBUG, "mod_licensing: Stripped 'key/' prefix from payload\n");
	}
	...
	if (!decoded_payload) {
		...
		if (key_info) {
			key_info->validation_msg = strdup("INVALID - base64 decode failed");
		}
		free(payload_copy);
		return SWITCH_STATUS_FALSE;
	}
	...
	if (!json_payload) {
		...
		if (key_info) {
			key_info->validation_msg = strdup("INVALID - JSON parse failed");
		}
	...
	/* Check if this is a full Keygen signed license (with "metadata" field) or a Keygen license file (direct metadata) */
	metadata_obj = cJSON_GetObjectItem(json_payload, "metadata");
	if (!metadata_obj && !license_obj && !policy_obj) {
		/* This is a Keygen license file - the entire JSON is the metadata */
		licensing_log(SWITCH_LOG_INFO, "mod_licensing: Processing Keygen license file (direct metadata format)...\n");
		metadata_obj = json_payload; /* The whole payload is metadata */
	}

	if (metadata_obj && cJSON_IsObject(metadata_obj)) {
		if (metadata_obj == json_payload) {
			licensing_log(SWITCH_LOG_INFO, "mod_licensing: Keygen license file detected\n");
		} else {
			licensing_log(SWITCH_LOG_INFO, "mod_licensing: Processing license metadata from Keygen signed license...\n");
		}

		/* Extract domain and fingerprint for validation */
		cJSON *domain = cJSON_GetObjectItem(metadata_obj, "domain");
		cJSON *fingerprint = cJSON_GetObjectItem(metadata_obj, "fingerprint");
		switch_bool_t has_domain = (domain && cJSON_IsString(domain) && domain->valuestring);
		switch_bool_t has_fingerprint = (fingerprint && cJSON_IsString(fingerprint) && fingerprint->valuestring);
		switch_bool_t domain_valid = SWITCH_FALSE;
		switch_bool_t fingerprint_valid = SWITCH_FALSE;
		...
		if (has_fingerprint) {
			licensing_log(SWITCH_LOG_INFO, "mod_licensing: License fingerprint: %s\n", fingerprint->valuestring);
			if (key_info) {
				key_info->fingerprint = strdup(fingerprint->valuestring);
			}

			if (fingerprint_matches(fingerprint->valuestring)) {
				licensing_log(SWITCH_LOG_INFO, "mod_licensing: Fingerprint validation PASSED (machine fingerprint matches license fingerprint)\n");
				fingerprint_valid = SWITCH_TRUE;
			} else {
				licensing_log(SWITCH_LOG_WARNING, "mod_licensing: Fingerprint validation FAILED (machine fingerprint '%s' does not match license fingerprint '%s')\n",
					globals.fingerprint, fingerprint->valuestring);
			}
		}

		/* License is valid if either domain OR fingerprint matches (or neither is specified) */
		if (has_domain || has_fingerprint) {
			if (domain_valid || fingerprint_valid) {
				licensing_log(SWITCH_LOG_INFO, "mod_licensing: License validation PASSED (domain: %s, fingerprint: %s)\n",
					domain_valid ? "PASS" : (has_domain ? "FAIL" : "not specified"),
					fingerprint_valid ? "PASS" : (has_fingerprint ? "FAIL" : "not specified"));
				status = SWITCH_STATUS_SUCCESS;
			} else {
				licensing_log(SWITCH_LOG_ERROR, "mod_licensing: License validation FAILED - neither domain nor fingerprint matched\n");
				if (key_info) {
					if (has_domain && has_fingerprint) {
						key_info->validation_msg = strdup("INVALID - domain and fingerprint mismatch");
					} else if (has_domain) {
			...
			/* No domain or fingerprint restriction - allow all */
			licensing_log(SWITCH_LOG_INFO, "mod_licensing: No domain or fingerprint restriction - license is unrestricted\n");
			status = SWITCH_STATUS_SUCCESS;
		}
		...
		if (modules) {
			if (cJSON_IsString(modules) && modules->valuestring) {
				...
				if (strcasecmp(modules->valuestring, "all") == 0) {
